...
if (!slideTextPath || !inspectPath || !outDir) {
  throw new Error('Uso: node u07_generate_template_plan.mjs <slide_text.md> <template-inspect.ndjson> <outDir>');
}
...
const sections = [...text.matchAll(/^## (U07-\d{3})\s*$\n([\s\S]*?)(?=^## U07-\d{3}\s*$|(?![\s\S]))/gm)];
if (sections.length !== 134) throw new Error(`Se esperaban 134 slides y se encontraron ${sections.length}.`);
  ...
  return {
    ...
    editTargets: [
      ...
      { action: 'add', newPrimitiveAllowed: true, mustNotOverlapInherited: true,
        ...
        reason: 'Agregar contenido editable de Unidad 7 dentro de la zona segura del layout aprobado.' },
    ],
  };
});

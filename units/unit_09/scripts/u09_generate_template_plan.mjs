...
if (!slideTextPath || !inspectPath || !outDir) {
  throw new Error('Uso: node u09_generate_template_plan.mjs <slide_text.md> <template-inspect.ndjson> <outDir>');
}
...
const sections = [...text.matchAll(/^## (U09-\d{3})[^\n]*\n([\s\S]*?)(?=^## U09-\d{3}[^\n]*\n|(?![\s\S]))/gm)];
if (sections.length !== 96) throw new Error(`Se esperaban 96 slides y se encontraron ${sections.length}.`);
  ...
  return {
    ...
    editTargets: [
      ...
      { action: 'add', newPrimitiveAllowed: true, mustNotOverlapInherited: true,
        ...
        reason: 'Agregar contenido editable de Unidad 9 dentro de la zona segura del layout aprobado.' },
    ],
  };
});
...
await fs.writeFile(path.join(outDir, 'deviation-log.txt'), [
  'DEVIATION LOG — Unidad 09',
  'Sin divisiones ni cambios de secuencia al iniciar la producción.',
  'Los recursos bloqueados se representan como contenido textual identificado, sin cifras ni fórmulas no aprobadas.',
].join('\n'), 'utf8');

...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("Uso: u09_create_preview_pdf.py <render_dir> <output.pdf>")
    render_dir = Path(sys.argv[1]).resolve()
    output = Path(sys.argv[2]).resolve()
    images = sorted(render_dir.glob("*.png"), key=slide_number)
    if len(images) != 96:
        raise ValueError(f"Se esperaban 96 renders y se encontraron {len(images)}")

    output.parent.mkdir(parents=True, exist_ok=True)
    page_size = (13.333333 * 72, 7.5 * 72)
    pdf = canvas.Canvas(str(output), pagesize=page_size, pageCompression=1)
    pdf.setTitle("Unidad 9 - Propagación del sonido - vista previa")

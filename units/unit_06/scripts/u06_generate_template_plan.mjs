...
if (!slideTextPath || !inspectPath || !outDir) {
  throw new Error('Uso: node u06_generate_template_plan.mjs <slide_text.md> <template-inspect.ndjson> <outDir>');
}
...
const sections = [...text.matchAll(/^### (U06-\d{3}) — ([^\n]+)\n([\s\S]*?)(?=^### U06-\d{3} — |(?![\s\S]))/gm)];
if (sections.length !== 117) throw new Error(`Se esperaban 117 slides y se encontraron ${sections.length}.`);
  ...
  return {
    ...
    editTargets: [
      ...
      { action: 'add', newPrimitiveAllowed: true, mustNotOverlapInherited: true,
        ...
        reason: 'Agregar contenido editable de Unidad 6 dentro de la zona segura del layout aprobado.' },
    ],
  };
});

...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("Uso: u10_create_preview_pdf.py <render_dir> <output.pdf>")
    render_dir = Path(sys.argv[1]).resolve()
    output = Path(sys.argv[2]).resolve()
    images = sorted(render_dir.glob("slide-*.png"), key=slide_number)
    if len(images) != 93:
        raise ValueError(f"Se esperaban 93 renders y se encontraron {len(images)}")

    output.parent.mkdir(parents=True, exist_ok=True)
    page_size = (13.333333 * 72, 7.5 * 72)
    pdf = canvas.Canvas(str(output), pagesize=page_size, pageCompression=1)
    pdf.setTitle("Unidad 10 - Ruidos - vista previa")
    pdf.setAuthor("UCASAL - Física Acústica")
    for image in images:

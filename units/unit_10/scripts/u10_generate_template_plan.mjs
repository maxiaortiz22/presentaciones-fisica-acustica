...
if (!slideTextPath || !inspectPath || !outDir) {
  throw new Error('Uso: node u10_generate_template_plan.mjs <slide_text.md> <template-inspect.ndjson> <outDir>');
}
...
const sections = [...text.matchAll(/^## (U10-\d{3})\s*\n([\s\S]*?)(?=^---\s*$|(?![\s\S]))/gm)];
if (sections.length !== 93) throw new Error(`Se esperaban 93 slides y se encontraron ${sections.length}.`);
...
const outputSlides = sections.map((m, index) => {
  const id = m[1];
  const body = m[2];
  const rawLayout = body.match(/### Layout\s*\n\s*([^\n]+)/i)?.[1]?.trim() || '';
  const layout = rawLayout.split(/\s*·\s*/)[0].replace(/`/g, '');
  ...
  return {
    ...
    editTargets: [
      ...
      { action: 'add', newPrimitiveAllowed: true, mustNotOverlapInherited: true,
        ...
        reason: 'Agregar contenido editable de Unidad 10 dentro de la zona segura del layout aprobado.' },
    ],
  };
});
...
await fs.writeFile(path.join(outDir, 'template-audit.txt'), [
  'TEMPLATE AUDIT — Unidad 10',
  'Plantilla aprobada: fisica_acustica_template_v01.pptx.',
  'Formato: 16:9, 1280×720 px equivalentes.',
  'Estructura inspeccionada: 2 masters y 27 layouts; se revisaron las 27 slides demostrativas.',
  ...
  'Los diagramas aprobados se reconstruyen desde diagram_source.json como formas y conectores editables.',
  'Los gráficos aprobados se insertan desde figure.svg con texto alternativo y caption.',
].join('\n'), 'utf8');

await fs.writeFile(path.join(outDir, 'deviation-log.txt'), [
  'DEVIATION LOG — Unidad 10',
  'Sin cambios de secuencia al iniciar la producción.',
  'U10-089 se divide visualmente mediante revelado estático dentro de una sola slide de respaldo; no se altera el ID.',
  'U10-088, U10-091 y U10-092 se representan como material condicionado o bloqueado, sin cifras ni protocolos no aprobados.',
].join('\n'), 'utf8');
